...
const PILLAR_ICON_MAP = { Puzzle, Gamepad2, GraduationCap, BarChart3, RefreshCw, Globe };
...
const PILLARS = [
  { icon:"Puzzle", title:"Project-Based", desc:"Every concept is applied immediately to a real project — not just drills." },
  { icon:"Gamepad2", title:"Gamified", desc:"Points, badges, and level-up moments keep kids motivated week after week." },
  { icon:"GraduationCap", title:"Expert Teachers", desc:"Friendly, patient instructors trained specifically to teach kids to code." },
  { icon:"BarChart3", title:"Progress Reports", desc:"Weekly progress updates sent to parents — always in the loop." },
  { icon:"RefreshCw", title:"Unlimited Revision", desc:"Kids can revisit any concept anytime — no one gets left behind." },
  { icon:"Globe", title:"Live Online", desc:"Interactive live sessions, not pre-recorded. Real human connection." },
];
            ...
            <motion.div key={i}
              ...
              <div className="md:w-7/12 space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-white text-base"
                    style={{ background:s.color,boxShadow:`0 6px 20px ${s.glow}` }}>{s.n}</div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-14">
          <motion.h2 initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }}
            ...
            Why kids <span style={{ color:T.green }}>love</span> learning here
          </motion.h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {PILLARS.map((p,i)=>(
            <motion.div key={i}
              ...
              <div className="w-10 h-10 mb-4 flex items-center justify-center rounded-xl" style={{ background: "rgba(16,185,129,0.1)" }}>
                {(() => { const I = PILLAR_ICON_MAP[p.icon]; return I ? <I className="w-5 h-5 text-emerald-600" /> : null; })()}
              </div>
